...
public class M extends View {
    ...
    void moveBalls() {
        for (int i = 0; i < N; i++) {
            if (i % 2 == 0) {
                x[i] = this.getWidth() / 2 + (int) (L[i] * vx[i] * Math.cos(a/2));
                y[i] = this.getHeight() / 2 + (int) (z * L[i] * vy[i] * Math.sin(a/2));
            } else {
                x[i] = this.getWidth() / 2 + (int) (L[i] * vx[i] * Math.cos(a/2));
                y[i] = this.getHeight() / 2 + (int) (L[i] * vy[i] * Math.sin(a/2));
            }
        }
        a = a + ha;
    }
